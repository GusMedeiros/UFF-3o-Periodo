...
void retira_um_sentido(TGrafo *g, int v1, int v2){
    ...
    if(!p) return;
    TVizinho *ant = NULL;
    TVizinho *alvo = p->prim_vizinho;

    while(alvo && alvo->id_vizinho != v2){
        ant = alvo;
        alvo = alvo->prox;
    }
    // no fim do while, ou não existe alvo, ou ele é o vizinho a ser removido.
    if(!alvo) return; // se não existe alvo, não há o que remover.
    if(!ant) { // se não existe anterior, o primeiro vizinho é o a ser removido.
        p->prim_vizinho = NULL;
        free(alvo);
        return;
    }
    // se chegou até aqui, é porque existe o alvo e ele não é o primeiro vizinho.
    ant->prox = alvo->prox;
    free(alvo);
}
*/
...
void imprime_vizinhos(TGrafo *vertice){
    TVizinho *viz = vertice->prim_vizinho;
    while(viz) {
        printf("%d -> ", viz->id_vizinho);
        viz = viz->prox;
    }
    printf("NULL\n");
}
...
TGrafo *retira_vertice(TGrafo *g, int v){
    TGrafo *ant = g;
    while(ant){
        if(ant->prox && ant->prox->id_vertice == v)
            break;
        ant = ant->prox;
    }
    if(!ant) // chegou ao final sem encontrar o vértice
        return g; // nao faz nada
    TGrafo *alvo = ant->prox;
    TVizinho *viz = alvo->prim_vizinho;
    while(viz){
        printf("viz atual = %d\n", viz->id_vizinho);
        imprime_vizinhos(alvo);
        printf("retirando aresta %d--%d\n", v, viz->id_vizinho);
        retira_aresta(g, v, viz->id_vizinho);
        viz = alvo->prim_vizinho;
    }
    ant->prox = alvo->prox;
    free(alvo);
    return g;
