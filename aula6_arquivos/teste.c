...
void altera_ultimo_nome(FILE *in, char* novo_nome) {
    ...
    fseek(in, 0, SEEK_END);
    fseek(in, ftell(in) - tamanho(), SEEK_SET);
...
int main(int argc, char** argv) {
    ...
    if ((out = fopen("funcionario.dat", "w+b")) == NULL) {
        ...
        //lendo o arquivo tdo novamente
        le_funcionarios(out);
        altera_nome(out, 1, "Taiane");
        altera_ultimo_nome(out, "Gustavo");
        le_funcionarios(out);
        //fecha arquivo
        fclose(out);
    }
