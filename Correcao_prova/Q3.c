...
int teste(TG *g, int origem, int destino, int k){
  printf("Testando %d, %d, %d\n", origem, destino, k);
  if(!g) return 0;
  if(origem == destino && k == 0) return 1;
  TG* temp;
  temp = TG_busca_no(g, origem);
  if(!temp) return 0;
  if(!TG_busca_no(g, destino)) return 0;
  TVIZ* aux1 = temp->prim_viz;
  while(aux1){
    if(teste(g, aux1->id_viz, destino, k-1)) return 1;
    aux1 = aux1->prox_viz;
  }
  printf("retornando 0\n");
  return 0;
...
int main(void){
  ...
  int k;
  printf("Digite origem, destino e k... ");
  scanf("%d%d%d", &n, &m, &k);
  printf("no %d alcanca o no %d em %d arestas? %d\n", n, m, k, teste(g, n, m, k));
  TG_libera(g);
  return 0;
}
